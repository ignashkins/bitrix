...
function CheckUserPassword($arF)
{
	...
	if(!array_key_exists("PASSWORD", $arF) || $arF["PASSWORD"] == '')
	{
		return "NOT";
	}
	$res = "PASSWORD";
	$pass = "";
	if(array_key_exists("USER_PASSWORD", $_REQUEST) && $_REQUEST["USER_PASSWORD"] <> '')
	{
		$pass = $_REQUEST["USER_PASSWORD"];
		$res = "PASSWORD_WRONG";
	}
	elseif(isset($_SESSION["WEBDAV_DATA"]["EXT_LINK_PASSWORD"]) && $_SESSION["WEBDAV_DATA"]["EXT_LINK_PASSWORD"] <> '')
	{
