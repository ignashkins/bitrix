	...
	if ($res = $db_res->GetNext())
	{
		do
		{
			...
			$aCols = array(
				...
				"WF_COMMENTS" => (mb_strlen($res["~WF_COMMENTS"]) <= $maxLength ?
					$res["WF_COMMENTS"]
					:
					htmlspecialcharsEx(mb_substr($res["~WF_COMMENTS"], 0, $maxLength - 3))."...".ShowJSHint($res["WF_COMMENTS"], array('return' => true))
				),
...
if ((!empty($_POST["HISTORY_ID"]) || !empty($_GET["history_id"])) && check_bitrix_sessid())
{
	...
	foreach($arID as $ID)
	{
		if($ID == '' || empty($arResult['VERSIONS'][$ID]))
			continue;
		$ID = intval($ID);
		...
		if($dr = $d->Fetch())
		{
			...
			elseif($dr["WF_PARENT_ELEMENT_ID"] <> '')
			{
				if ((CWebDavIblock::CheckRight($arResult["ELEMENT"]["PERMISSION"], "element_edit") < "W")
					&& !(
						($dr["WF_STATUS_ID"] > 1)
