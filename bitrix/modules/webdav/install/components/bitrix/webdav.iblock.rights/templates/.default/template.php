			...
			<div class="<?=(!$inherited ? 'wd-toggle-edit quick-view' : 'wd-inherited')?> wd-section">
				...
				<?if ($inherited && ($arRight['ENTITY_TYPE'] == 'iblock' || $arRight['ENTITY_TYPE'] == 'section')) { ?>
					<? if (! $arRight['ENTITY_SELF']) { ?>
						<a target="<?=$arRight['ENTITY_SOURCE_URL']?>" <?/*ondblclick="return BX.WebDavRightsDialog.ShowDialog(this);"*/?> class="wd-inherited-source">(<?=GetMessage('WD_INHERITED_FROM_'.mb_strtoupper($arRight['ENTITY_TYPE']), array('#NAME#' => htmlspecialcharsEx($arRight['ENTITY_SOURCE_NAME'])))?>)</a>
					<? } else { ?>
						<span></span>
					<? } ?>
				<? } ?>
