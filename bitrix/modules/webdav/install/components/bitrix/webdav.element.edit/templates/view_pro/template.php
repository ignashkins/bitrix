...
";
	...
	$lhe = ob_get_clean();
if ($elementPreviewText == '') $elementPreviewText="&nbsp;";
	$arFields[] = array("id" => "PREVIEW_TEXT", "name" => GetMessage("WD_DESCRIPTION"), "type" => "custom", "value" => 
		...
		if ($arParams["USE_WORKFLOW"] == "Y")
		{
			...
			if ($arParams["SHOW_WORKFLOW"] != "N")
			{
				...
				if (mb_strlen($sCurStatus) < 1) $sCurStatus = '&nbsp;';
			...
			$arData["WF_COMMENTS"] .= "<div class=\"quick-view wd-toggle-edit wd-wfcomments\">".(($arResult["ELEMENT"]["WF_COMMENTS"] <> '') ? $arResult["ELEMENT"]["WF_COMMENTS"] : "&nbsp;")."</div>";
		}
