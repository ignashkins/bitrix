...
if($arParams["FILTER_NAME"] == '' || !preg_match("^[A-Za-z_][A-Za-z01-9_]*$", $arParams["FILTER_NAME"], $arMatches))
{
	$arrFilter = array();
}
else
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		while ($arItem = $rsElement->Fetch())
		{
			...
			if($arParams["PREVIEW_TRUNCATE_LEN"]>0)
			{
				...
				while(mb_substr($arItem["PREVIEW_TEXT"], $end_pos, 1) != " " && $end_pos < mb_strlen($arItem["PREVIEW_TEXT"]))
					$end_pos++;
				if($end_pos < mb_strlen($arItem["PREVIEW_TEXT"]))
					$arItem["PREVIEW_TEXT"] = mb_substr($arItem["PREVIEW_TEXT"], 0, $end_pos)."...";
			}
			...
			$arItem["FILE_EXTENTION"] = htmlspecialcharsbx(mb_strtolower(strrchr($arItem['~NAME'], '.')));
			$arItem["FIELDS"] = array();
