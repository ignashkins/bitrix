			...
			if ($dbWDFile)
			{
				...
				while ($arWDFile = $dbWDFile->Fetch())
				{
					...
					if (isset($arIBlock[$arWDFile['IBLOCK_ID']]))
					{
						...
						{
							$name = $res["NAME"];
							if (($i == 0) && !!$res["SOCNET_GROUP_ID"] && CModule::IncludeModule('socialnetwork') && GetMessage('SONET_GROUP_PREFIX') <> '')
							{
...
function WDUFHasHistoryModify($arWDFile, $postDatePublish)
{
	...
	if (CWebDavEditDocBase::isEditable($arWDFile['NAME']))
	{
		...
		if(mb_stripos($arWDFile['IBLOCK_CODE'], 'shared') !== false)
		{
			$filter  = array(
...
function WDUFGetPathOptions($path, $IBLOCK_ID, $arSection, $arElement = array())
{
	...
	{ // old version
		$entity = ((mb_strpos($path, "#user_id#") !== false) || (mb_strpos($path, "#USER_ID#") !== false) ? "user" : (
			((mb_strpos($path, "#group_id#") !== false) || (mb_strpos($path, "#GROUP_ID#") !== false)) ? "group" : "lib"));

		if (mb_strpos($path, "#SITE_DIR#") !== false)
		...
		else if (CModule::IncludeModule('extranet') && (CExtranet::GetExtranetSiteID() == SITE_ID))
		{
			...
			if ( $arExtranetSite && (mb_strpos($path, $arExtranetSite["DIR"]) === false))
			{
				if($defSite === false)
				{
				...
				if ($entity == "user")
				{
					...
					if (mb_strpos($path, $intranet_path) === 0)
		...
		if ($entity != "lib")
		{
			$SEF_FOLDER = mb_substr($path, 0, mb_strpos(mb_strtolower($path), ($entity == "user"? "user/#user_id#/files" : "group/#group_id#/files")));
