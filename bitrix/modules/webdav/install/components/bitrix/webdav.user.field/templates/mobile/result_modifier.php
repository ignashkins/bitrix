...
if (mb_strpos($this->__page, "view") === 0)
{
	...
	foreach ($arResult['FILES'] as $id => $arWDFile)
	{
		if (CFile::IsImage($arWDFile['NAME'])) // we don`t use content_type for this checking because revision 0fa6437117d2
		{
			...
			$arResult["FILES"][$id]["THUMB_SRC"] =
				$arWDFile["PATH"].(mb_strpos($arWDFile["PATH"], "?") === false ? "?" : "&")."cache_image=Y";
			...
			{
				$arResult["FILES"][$id]["PATH"] .= (mb_strpos($arResult["FILES"][$id]["PATH"], "?") === false ? "?" : "&")."cache_image=Y&width=".$max_dimension."&height=".$max_dimension;
			}
