...
$arParams["PERMISSION"] = mb_strtoupper(trim($arParams["PERMISSION"]));
...
********************************************************************/
	...
	if (!function_exists('get_response_code'))
	{
		...
		{
			$line = mb_substr($in, 0, mb_strpos($in, "\n"));
	...
	if (!function_exists('do_http_request'))
	{
		function do_http_request($method,  $path, $hostname=null, $hostport=null, $agent="Mozilla/5.0 (X11; U; Linux i686; ru; rv:1.9.2.12) Gecko/20101027 Ubuntu/10.04 (lucid) Firefox/3.6.12")
		{
			...
			if ($disabled === null)
			{
				...
				$disabled = ! (mb_strpos($disabled_functions, 'fsockopen') === false);
			}
	...
	if (!IsModuleInstalled('bitrix24'))
	{
		if ($in = do_http_request('OPTIONS', '/docs/shared/'))
		{
			...
			if (mb_strpos($hResponse['Server'], "Apache") === false)
			{
				//"Apache do not recive DAV protocol requests. Check the front-end server, if any.";
			}
			if (!(isset($hResponse['X-Powered-CMS']) && (mb_strpos($hResponse['X-Powered-CMS'], 'Bitrix Site Manager') !== false) ))
			{
			...
			{
				if (mb_strpos($hResponse['WWW-Authenticate'], 'NTLM') !== false)
					$serverParams['AUTH_MODE'] = 'NTLM';
				elseif (mb_strpos($hResponse['WWW-Authenticate'], 'Basic') !== false)
					$serverParams['AUTH_MODE'] = 'BASIC';
				elseif (mb_strpos($hResponse['WWW-Authenticate'], 'Digest') !== false)
