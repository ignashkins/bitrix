...
$sapi = mb_strtolower(php_sapi_name());
...
if ($arParams["RESOURCE_TYPE"] == "FOLDER")
{
	...
	foreach ($arDefaultUrlTemplates404 as $url => $value)
	{
		...
		if (!$prevComponentPage && mb_strpos($currentPageTemplate, "#PATH#") !== false && $currentPageTemplate != "#PATH#")
		{
...
else
{
	...
	elseif ($componentPage == "section_edit" && isset($_REQUEST["use_light_view"]) && mb_strtoupper($_REQUEST["use_light_view"]) == "Y")
	{
		$componentPage = "section_edit_simple";
	}
	elseif ($componentPage == "element_upload" && isset($_REQUEST["use_light_view"]) && mb_strtoupper($_REQUEST["use_light_view"]) == "Y")
	{
	...
	if ($componentPage == "sections" && mb_strpos($_SERVER["REQUEST_URI"], "dialog=Y") !== false)
	{
...
********************************************************************/
	...
	if ($arParams["RESOURCE_TYPE"] == "FOLDER" && $componentPage == "element_view")
		$ob->SendHistoryFile($arResult["VARIABLES"]["ELEMENT_ID"]);
	elseif ($arParams["RESOURCE_TYPE"] == "FOLDER" && in_array(mb_substr($componentPage, 0, 7), array("section", "element")))
