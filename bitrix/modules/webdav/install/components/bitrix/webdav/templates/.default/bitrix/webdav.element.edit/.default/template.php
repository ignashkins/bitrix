...
else
{
	...
	if (mb_strtoupper($arParams["ACTION"]) == 'PULL') {
	...
	elseif ($arParams["USE_BIZPROC"] == "Y")
	{
		...
		if (!empty($arResult["ELEMENT"]["~arDocumentStates"]))	
		{
			foreach ($arResult["ELEMENT"]["~arDocumentStates"] as $key => $arDocumentState)
			{
				...
				$arDataTmp = array(); 
				$bizProcIndex++;
				if (($arDocumentState["WORKFLOW_STATUS"] == '' && $arDocumentState["ID"] > 0) ||
					($arDocumentState["ID"] <> '' && !$arDocumentState["ViewWorkflow"]))
					continue;
				...
				$id = ($arDocumentState["ID"] <> '' ? $arDocumentState["ID"] : $arDocumentState["STATE_NAME"]); 
				if ($arDocumentState["STATE_NAME"] <> '' || 
					($arDocumentState["ID"] <> '' && $arDocumentState["WORKFLOW_STATUS"] <> ''))
				{
					$sData = ($arDocumentState["STATE_TITLE"] <> '' ? $arDocumentState["STATE_TITLE"] : $arDocumentState["STATE_NAME"]); 
...
<?
if (mb_strtoupper($arParams["ACTION"]) == 'PULL' || (mb_strpos($_SERVER['REQUEST_URI'], 'element_active_tab=tab_main') !== false)): ?>
