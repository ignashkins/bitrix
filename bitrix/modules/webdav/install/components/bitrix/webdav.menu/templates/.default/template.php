...
$bIsIE = (mb_strpos($res, "opera") === false && mb_strpos($res, "msie") !== false);
$bIsFF = (mb_strpos($res, "firefox") !== false);
$ob = $arParams['OBJECT'];
...
if (mb_strpos($arParams["PAGE_NAME"], "WEBDAV_BIZPROC_WORKFLOW") !== false)
{
	if ($arParams["USE_BIZPROC"] == "Y" && $ob->CheckRight($arParams["PERMISSION"], 'element_edit') >= "W" && IsModuleInstalled("bizprocdesigner"))
	{
		$arButtons[] = array(
			...
			"LINK" => $arResult["URL"]["WEBDAV_BIZPROC_WORKFLOW_EDIT"].(mb_strpos($arResult["URL"]["WEBDAV_BIZPROC_WORKFLOW_EDIT"], "?") === false ? "?" : "&").
				"init=statemachine",
			"ICON" => "btn-list"); 
		$arButtons[] = array(
...
else
{
	if ($this->__component->__parent)
	{
		...
		if ($link)
		{
			if(mb_strrpos($link, '/') != (mb_strlen($link) - 1) )
			{
				$link .= '/';
			}
			array_unshift($arButtons, array(
...
{
	if (mb_strpos($arButton['ICON'], 'settings') !== false)
	{
